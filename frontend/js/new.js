jQuery(document).ready(function($) {
	...
	$(".ty-btn").click(function(){
		var el = $(".section_cldr").next();
		$([document.documentElement, document.body]).animate({
			scrollTop: el.offset().top - 100
		}, 1000);
	});

	function stripesSize() {
		if ($('.guide__row-stripes').length) {
			$('.guide__row-stripes').each(function() {
				let adj = (($(this).get(0).getBoundingClientRect().width - $(this).width()) / 2 + 2).toFixed();
				//console.log(adj);
				$(this).get(0).style.setProperty('--stripesR', adj + 'px');
			});
		}
	}
});
